...
def __(
    ...
    class DriftCorrectStep(CalStep):
        ...
        def calc_from_df(self, df):
            ...
            df2 = df.select(
                (pl.col(uncorrected) * (1 + m * (pl.col(indicator) - b))).alias(self.output)
            ).with_columns(df)
            return df2
...
def __(
    ...
    @dataclass(frozen=True)
    class Channels:
        channels: collection.OrderedDict[int, Channel]
        description: str
        ...
        def dfg(self, exclude="pulse"):
            ...
            for key, channel in self.channels.items():
                ...
                dfs.append(df.with_columns(key_series, ch_series))
...
@app.cell
def __(Channels, channel):
    data = Channels({1: channel, 2: channel}, "testing Channels")
    return data,
...
@app.cell
def __(data2):
    results = data2.linefit_joblib("MnKAlpha", col="energy_5lagy_dc")
    return results,
...
@app.cell
def __(data2, mo, plt):
    data2.linefit("MnKAlpha", col="energy_5lagy_dc").plotm()
    mo.mpl.interactive(plt.gcf())
    return
...
def __(data2, mo):
    _keys = list(str(key) for key in data2.channels.keys())
...
def __(data2, dropdown_ch, dropdown_col, mo, plt):
    ...
    data2.channels[int(_ch)].linefit("MnKAlpha", col=_col).plotm()
...
def __(data2):
    ...
    step_summary(data2.channels[1])
    return step_summary,


@app.cell
def __(channel4, np):
    _a = channel4.df["shift1"].to_numpy()
    _b = channel4.df_history[1]["shift1"].to_numpy()
    np.shares_memory(_a, _b)
    return
